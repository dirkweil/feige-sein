...
public class ShopTestDataService implements TestDataService
{
  ...
  {
    this.artikelRepository.persist(ARTIKEL_FLUG_DUS_JFK);
    this.artikelRepository.persist(ARTIKEL_HOTEL_MARRIOTT_NYC);
  }

  @Override
  public void unloadTestData() throws Exception
  {
    this.entityManager.createNativeQuery("delete from " + Bestellposition.TABLE_NAME).executeUpdate();
    this.entityManager.createNativeQuery("delete from " + Bestellung.TABLE_NAME).executeUpdate();
    this.entityManager.createNativeQuery("delete from " + Artikel.TABLE_NAME).executeUpdate();
  }
